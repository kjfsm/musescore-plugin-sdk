...
export interface EngravingItem extends ScoreElement {
  ...
  label: string;
  glissText: string;
  glissandoStyle: number;
  groups: unknown;
  lineStyle: number;
  lineColor: QColor;
  lineWidth: number;
  ornamentStyle: number;
  ...
  endFontFace: string;
  notelinePlacement: number;
  voiceAssignment: number;
  ...
  path: unknown;
  playTechType: number;
  tempoChangeType: number;
  tempoEasingMethod: number;
  ignoredStrings: unknown;
  tiePlacement: number;
  minLength: unknown;
  partialSpannerDirection: number;
  preset: string;
