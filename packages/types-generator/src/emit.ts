...
const HEADER = `// Generated by @kjfsm/musescore-plugin-sdk-types-generator. Do not edit by hand.
// Run \`pnpm generate:types\` from the repo root to regenerate.
`;

// Qt geometry types that aren't part of the Plugin API but appear in property/method types.
...
export function emit(input: EmitInput): EmitOutput {
  ...
  // Auxiliary type aliases for referenced types we couldn't model.
  ...
  for (const ref of [...referencedTypes].sort()) {
    ...
    // Unknown Qt enum-like — model as number (apiv1 surfaces enums as ints).
    auxiliary.push(`export type ${ref} = number;`);
  }
  if (auxiliary.length > 0) {
    pluginApiLines.push("// Auxiliary types referenced from the Plugin API but not modeled.");
