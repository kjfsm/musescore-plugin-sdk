...
4. `emit.ts` — `plugin-api.ts` と `enums.ts` を生成する。同じクラスが複数のヘッダに登場する場合はクラスを統合（プロパティ・メソッドをマージ）し、`extends BaseClass` で継承を表現、プロパティとメソッドの名前衝突は `Omit<...>` で解消する。マッピング不能な Qt 型（例: `QPointF`）はインラインでエイリアス化する。enum は**値を持たない型のみ**として出力する（apiv1 では int として現れ、MuseScore のバージョン間で並び替わりうるため）: メンバ名は `export type <Enum>Name = "A" | "B" | ...` のユニオン、値は `export type <Enum> = EnumValue<"<Enum>">`（`EnumValue<Tag> = number & { readonly __enum: Tag }` でブランド化された number）。`export const <Enum> = { ... }` は出力しない — `<Enum>.MEMBER` のような値使用はコンパイルエラーになり、異なる enum 同士の値比較（ブランド不一致）も型エラーになる。`qmlpluginapi.h` の `PluginAPI` クラスはホスト型 `MuseScore` にリネームして出力し、`DECLARE_API_ENUM(QmlName, _, ::Enum)` は実行時 enum オブジェクトを表す `readonly QmlName: RuntimeEnum<EnumName, Enum>` として生成する（生成 enum が無いものはスキップ＋警告）。
...
- **enum 値は焼き込まず、実行時のホスト enum を使う。** `Element` / `NoteType` / `BarLineType` などは `MuseScore { }` のプロパティ（実行中の版が値を解決する enum）。`enums.ts` の生成 enum（例: `BarLineType`）は**値を持たない型のみ**なので、`BarLineType.END` のように値として使おうとすると（インポート元にかかわらず）コンパイルエラーになる。代わりに `el.type === host.Element.NOTE` と書く（キーは生成 enum の `<Enum>Name` で型チェック、値は実行時に解決）。生成 enum の値はブランド化されているため、異なる enum 同士の取り違え（例: `el.type === host.NoteType.NORMAL`）も型エラーで検出できる。helpers に個別 enum（`host.NoteType` 等）を渡す関数もある（`RuntimeEnum<Name, Value>` / `ElementEnum` / `NoteTypeEnum` で受ける）。要素側に名前がある型（`ElementType`）は helpers の述語（`isNote` 等）のように `el.name` 文字列でも判定できる。
