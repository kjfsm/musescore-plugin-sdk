...
|---|---|---|
| `@kjfsm/types` | TypeScript type definitions for the MuseScore 4 Plugin API. | ✅ |
| `@kjfsm/types-generator` | Internal: regenerates `@kjfsm/types` from `musescore/MuseScore` C++ headers. | ❌ |

## Repository layout

```
packages/
  types/                 # @kjfsm/types — published type defs
...
├── src/logic.ts        # TypeScript with `import type { Score } from "@kjfsm/types"`
